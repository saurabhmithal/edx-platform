...
PDF_RECEIPT_DISCLAIMER_TEXT = "THE SITE AND ANY INFORMATION, CONTENT OR SERVICES MADE AVAILABLE ON OR THROUGH " \
                              "THE SITE ARE PROVIDED \"AS IS\" AND \"AS AVAILABLE\" WITHOUT WARRANTY OF ANY KIND" \
                              " (EXPRESS, IMPLIED OR OTHERWISE), INCLUDING, WITHOUT LIMITATION, ANY IMPLIED" \
...
PDF_RECEIPT_FOOTER_TEXT = "EdX offers online courses that include opportunities for professor-to-student and" \
                          " student-to-student interactivity, individual assessment of a student's work and, for" \
                          " students who demonstrate their mastery of subjects, a certificate of achievement or" \
                          " other acknowledgment."
PDF_RECEIPT_TAX_ID = "46-0807740"
PDF_RECEIPT_TAX_ID_LABEL = "edX Tax ID"
PDF_RECEIPT_TERMS_AND_CONDITIONS = "Enrollments:\nEnrollments must be completed within 7 full days from the" \
                                   " course start date.\nPayment Terms:\nPayment is due immediately. Preferred" \
                                   " method of payment is wire transfer. Full instructions and remittance details" \
                                   " will be included on your official invoice. Please note that our terms are" \
                                   " net zero. For questions regarding payment instructions or extensions," \
                                   " please contact onlinex-registration@mit.edu and include the words \"payment" \
                                   " question\" in your subject line.\nCancellations:\nCancellation requests must" \
                                   " be submitted to onlinex-registration@mit.edu 14 days prior to the course" \
                                   " start date to be eligible for a refund. If you submit a cancellation request" \
                                   " within 14 days prior to the course start date, you will not be eligible for" \
                                   " a refund. Please see our Terms of Service page for full details." \
                                   "\nSubstitutions:\nThe MIT Professional Education Online X Programs office" \
                                   " must receive substitution requests before the course start date in order" \
                                   " for the request to be considered. Please email onlinex-registration@mit.edu" \
                                   " to request a substitution.Please see our Terms of Service page for our" \
                                   " detailed policies, including terms and conditions of use."
...
class TestPdfFile(unittest.TestCase):
    ...
    def test_pdf_receipt_configured_generation(self):
        ...
        self.assertTrue(any(str(self.context['total_cost']) in s for s in pdf_content))
        self.assertTrue(any(str(self.context['payment_received']) in s for s in pdf_content))
        self.assertTrue(any(str(self.context['balance']) in s for s in pdf_content))
